# Side Quest

### An RPG quest and encounter website Maybe Magazine.

Since the time of Gary Gygax; Dungeon Masters, Games Masters and Story tellers have been searching for quest and encounter ideas.

Even now the Roleplaying table top platform can be played online through sites as Roll20, there is still few small quests and encpunters to aid our dungeon masters.

That is where we at **SIDEQUEST** Leap in to provide a teasing snippet now and with your help extend our repitour to provide more quest ideas in the future. 


## UX

### Who is the site for? 

* For the owner a Table top Roleplay game Dungeon Master wanting to share quest and Encounter Ideas.
* For Table Top Roleplay Game Dungeon Masters, Game Masters and Storytellers.

### What do they want to achieve?

* The owner Wants to share their ideas of good quick small quests and encounters and recieve feedback and ideas from the community.
* The users want to recieve ideas for quick easy quests and share their own ideas, stories and quests.

### Planning
1. I started by choosing this idea out of the few that I had using the table points system.
1. I then used the table points system and choice graph to simplify down to 20 or so pages.
1. I then took the obviouly good advice of my mentor to simplify down to the 8 pages I now have.
1. I have provided my [wireframe](plans/side-quest-wireframe.bmpr)
    * This does not display in the git workspace so Please let me know if I need to provide a different link.

### User Stories

* As the website owner I want to share quest story ideas with fellow games masters in the online RPG community,
    * so other games masters can find quests quickly and easily for their parties entertainment.

* As a user I want to Access an idea for a quest for my party of Roleplaying characters.
    * So I can provide an entertaining quest to my players.

* As a user I want to Look for Quests for Dungeons and Dragons platform of RPG
    * So I can use the quest idea in the Dungeons and Dragons platform.

* As a user I want to Look for Quests for Pathfinder platform of RPG
    * So I can use the quest idea in the Pathfinder platform.

* As a user I want to Look for low level Quests
    * So I can use the quest idea for my low level party.

* As a user I want to Look for medium level Quests
    * So I can use the quest idea for my medium level party.

* As a user I want to Look for high level Quests
    * So I can use the quest idea for my high level party.

* As a user I want links to Side Quests Social media.
    * So that I can see Side Quests social media post for information

* As a user I want to be able to provide feedback and subscribe to a newsletter,
    * So that the site will be more user centric in future and I can receive updates on new quests available and site developments.

* As a user I want to submit my ideas for plot hooks, quest Ideas, platforms, Adversaries and rewards.
    * So the site can display them to the growing community.

## Features

### Existing Features

1. Feature- Landing Page- a fun introducton to the site to put the user at ease so they can feel free to explore the rest of the site.
1. Feature- Available Quests- Quest suggestions for DM's To view and use for their parties.
    * Low level, quests for low level parties.
    * medium level quests for medium level parties.
    * high level quests for high level parties.
    * Quests for Dungeons & Dragons platform.
    * Quests for Pathfinder platform.
1. Feature- Adversary PDF statistic sheet download buttons so users can download the adversary statisic sheet and have the information on hand easily.
1. Feature- Form for Email address so users can recieve updates on new quests available and site developments.
1. Feature- Check buttons and text input so users can submit their ideas for plot hooks, quest Ideas, platforms, Adversaries and rewards.


### Features to be Implemented

#### future releases
* more quest
* more platforms
* interactive elements
* user content 

## Techologies Used

* HTML5
* CSS3
* [Bootstrap](https://getbootstrap.com/)
* [font awsome](https://fontawesome.com/)
* [google fonts](https://fonts.google.com/)

## Testing

I was most daunted by this process due to the views of testing done on other projects and it took a while for me to get my head around the testing processes. I hope this will help me with future testing.

![project-testing-1.pdf](assets/pdf/project-testing-1.pdf)

none yet

## Deployment

The Deployment of this project is through [Github](https://github.com/) even the code was written on the Git IDE.

When the Site was ready for deployment the following steps were taken.
* How to deploy was looked up.
* In the Sidequest repository the settings tab was selected.
* some scrolling was required to get to the Github pages section where upon selecting to continue the following site was issued to host the site. 

https://theforkbeard.github.io/sidequest/

Should there be a need to operate the site or use the code from a seperate location the code can be cloned or downloaded.
* select the green "clone or Download".
* choose whether to copy, open in desktop, or download zip.
* install into own device using manfacturers instructions.

## Credits

### content

* [basilisk.pdf](assets/pdf/basilisk.pdf)
* [giant-crab.pdf](assets/pdf/giant-crab.pdf)
* [Goblin.pdf](assets/pdf/Goblin.pdf)
* [goblin-acolyte.pdf](assets/pdf/goblin-acolyte.pdf)
* [goblin-guard.pdf](assets/pdf/goblin-guard.pdf)
* [hobgoblin.pdf](assets/pdf/hobgoblin.pdf)
* [orc-guard.pdf](assets/pdf/orc-guard.pdf)
* [orc-miner.pdf](assets/pdf/orc-miner.pdf)
* [orc-war-chief.pdf](assets/pdf/orc-war-chief.pdf)
* [shambling-mound.pdf](assets/pdf/shambling-mound.pdf)
* [stone-cursed.pdf](assets/pdf/stone-cursed.pdf)
* [swarm-of-bats.pdf](assets/pdf/swarm-of-bats.pdf)

copied from below rules books and converted to PDF.
* [Dungeons and Dragons](https://dnd.wizards.com/)
* [Pathfinder](https://paizo.com/pathfinder)
* [Roll 20](https://roll20.net/)

### Media

Photos- 
* [Herocave.png](assets/images/Herocave.png)
* [Mysterious.png](assets/images/Mysterious.png)
* [Questposter.png](assets/images/Questposter.png)

From 

[shutterstock](https://www.shutterstock.com/) Then edited to achieve desired results

* [Contactusback.jpg](assets/images/Contactusback.jpg)

From Paul Smith personal stock
	
Drawings-

* [elminster.png](assets/images/elminster.png)

From Wizards of the Coast [D&D](https://dnd.wizards.com/) Then edited to achieve desired results

* [Stash.png](assets/images/Stash.png) 

From [Roll20](https://roll20.net/) System token 

* [lowmap.png](assets/images/lowmap.png)

* [Medmap.png](assets/images/Medmap.png)

* [Highmap.png](assets/images/Highmap.png)

Drawn by Paul Smith

### Acknowledgements

The following organistions deserve to be praised for how they aided this project.

* [Dungeons and Dragons](https://dnd.wizards.com/) For not only inspiring this site but though its various incarnations in the past provided myself many hundreds of hours of reading material over the last four decades.
* [Pathfinder](https://paizo.com/pathfinder) For keeping a popular format of the game alive.
* [Roll 20](https://roll20.net/) For Showing me how to play A much loved game from the comfort of my own home.
* And Of Course [**CODE INSTITUTE**](https://courses.codeinstitute.net/) For guiding me through this exciting course.

**With thanks!**





